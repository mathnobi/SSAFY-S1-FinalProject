<template>
  <div class="savings-container">
    ...
    <table class="deposit-table">
      ...
      <tbody>
        <SavingsList
          v-for="savings in filteredSavings"
          :key="savings.id"
          :savings="savings"
...
const uniqueBanks = computed(() => {
  const banks = counterStore.savings.map(savings => savings.kor_co_nm)
  return [...new Set(banks)].sort()
})

// 필터링된 예금 상품 목록
const filteredSavings = computed(() => {
  let filtered = counterStore.savings

  if (selectedBank.value) {
    filtered = filtered.filter(savings => savings.kor_co_nm === selectedBank.value)
  }
